...
(function($) {
  ...
  //   var active_page = $.mobile.activePage.height();

  //   _messagePopup('viewport_height: ' + viewport_height, false);
  //   _messagePopup('body: ' + $('body').height(), false);
  //   _messagePopup('active_page: ' + active_page, false);

  //   var content_height = viewport_height - header.outerHeight() - footer.outerHeight();
  //   if ((content.outerHeight() - header.outerHeight() - footer.outerHeight()) <= viewport_height) {
  //     content_height -= (content.outerHeight() - content.height());
  //   }
  //   _messagePopup('content_height: ' + content_height, false);
  //   $('body').height(content_height);
  //    _messagePopup('body: ' + $('body').height(), true);
  //   return content_height;
  // }

  // $(window).resize(function(event) {
  //   // _getRealContentHeight();
  // });

  // Transaction success callback
  function _dbSuccessHandler(tx) {
    tx.executeSql('SELECT timestamp FROM variable WHERE name = "cron"', [], _selectCronVariableCB, _errorHandler);
  }
  ...
  function _queryExclude() {
    ...
    if (queryExcluded === true) {
      apApp.settings.dbPromiseTracker.transaction(_dbQuery, _errorHandler);
    }
  }
    ...
    $(document)
          ...
          case 'click':
            ...
            apApp.settings.dbPromiseTracker.transaction(function(tx) {
              ...
              tx.executeSql('UPDATE goal_index SET completed = ?, updated = ? ' +
                ...
                [data.completed, timestp, apApp.settings.profileUID, data.cid, data.gid],
                function(tx, results) {
                  if ($goal.hasClass('checked')) {
            ...
            switch (e.type) {
              ...
              case 'swiperight':
                if (!$(this).hasClass('hidden')) {
                  if ($.mobile.activePage.attr('id') == 'edit-child') {
                    apApp.settings.dbPromiseTracker.transaction(function(tx) {
                      tx.executeSql('DELETE FROM child_index WHERE cid = ? AND uid = ?',
                        [data.cid, data.uid], function(tx, results) {
      ...
      .on('click', 'button.submit-goal', function(e) {
        ...
        if (!$form.find('div.input-goal input').val()) {
          ...
          apApp.settings.dbPromiseTracker.transaction(function(tx) {
            var timestp = parseInt(new Date().getTime() / 1000); // timestamp
            tx.executeSql('INSERT INTO goals (gid_origin, uid_origin, uid, ' +
              'title, featured, updated, created, status) ' +
              'VALUES (0, 0, ?, ?, 0, ?, ?, ?)', [apApp.settings.profileUID, data.title, timestp, timestp, data.status],
              function(tx, results) {
                data.gid = results.insertId;
                tx.executeSql('INSERT INTO goal_index (gid, cid, ' +
                  'uid, completed, updated) ' +
                  'VALUES (?, ?, ?, 0, ?)', [data.gid, data.cid, apApp.settings.profileUID, timestp],
                  function(tx, results) {}, _errorHandler);
                tx.executeSql('INSERT INTO age (entity_id, type, ' +
                  'age, delta) ' +
                  'VALUES (?, "goal", ?, 0)', [data.gid, data.age],
                  function(tx, results) {}, _errorHandler);
                tx.executeSql('INSERT INTO topic (entity_id, type, ' +
                  'topic, delta) ' +
      ...
      if ($('#add-child form .error').get(0)) {
        ...
        $.mobile.loading('show');
          tx.executeSql('INSERT INTO childs (cid_origin, uid, first_name, ' +
            ...
            _saveChildSuccessCB, _errorHandler);
        }, _errorHandler);
      }
      e.preventDefault();
    });
  ...
  function _dbQuery(tx) {
    ...
    tx.executeSql('SELECT u.uid, u.image_path, u.name, u.address, u.email ' +
      'FROM users AS u', [], _selectUsersSuccessCB, _errorHandler);
    ...
    tx.executeSql('SELECT c.image_path, c.cid, c.first_name, c.age, ' +
      ...
      'ORDER BY c.cid DESC, gi.completed ASC', [], _selectChildSuccessCB, _errorHandler);
    ...
    tx.executeSql('SELECT c.image_path, c.cid, c.first_name, c.age, g.title, ' +
      ...
      'LEFT JOIN goal_index AS gi ON gi.gid = g.gid ' +
      'WHERE a.type = "goal" ' +
      'ORDER BY g.title ASC', [], _selectSearchPageSuccessCB, _errorHandler);
  ...
  function _selectUsersSuccessCB(tx, results) {
    ...
    if (len) {
      for (var i = 0; i < len; i++) {
        ...
        $('ul.persons-inline li.add-new').before(inlinePerson);
    ...
    $('input.profile-input[type="text"]').on('change', function(e) {
      ...
      apApp.settings.dbPromiseTracker.transaction(function(tx) {
        ...
        tx.executeSql('UPDATE users SET name = ?, address = ?, email = ?, ' +
          'image_path = ?, updated = ? ' +
          'WHERE uid = ?', [data.name, data.address, data.email,
            data.image_path, updatedTimestamp,
            apApp.settings.profileUID
          ],
          function() {
      ...
      $.each(children, function(cid, child) {
        if (child != undefined) {
          var article = _getHtml('article', child, options),
            ...
            myChildren = _getHtml('myChildren', child),
  ...
  function _captureProfilePhoto(photolibrary) {
    ...
    navigator.camera.getPicture(_captureProfilePhotoSuccess, _errorHandler, options);
  }
  ...
  // function _saveChildIndexSuccessCB(tx, results, child) {
  //   child.goals = [];
  //   tx.executeSql('SELECT DISTINCT(g.gid), g.title, gi.completed ' +
  //     'FROM goals g ' +
  //     'LEFT JOIN goal_index AS gi ON gi.gid = g.gid ' +
  //     'LEFT JOIN age AS a ON a.entity_id = g.gid ' +
  //     'WHERE a.age = ? ' +
  //     'ORDER BY g.title ASC', [child.age], function(tx, results) {
  ...
  //         });
  //       }
  //       _addNewChild(tx, child);
  //     }, _errorHandler);
  // }

  // function _addNewChild(tx, child) {
  //   // set variables and get content.
  //   var options = {
  //     'isNew': true,
  //     'articleLinked': true,
  //     'pagerName': 'children-'
  ...
  //   }
  //   var article = _getHtml('article', child, options),
  //     myChildren = _getHtml('myChildren', child, options),
  //     childPage = _getHtml('childPage', child, options),
  //     addGoalPage = _getHtml('addGoalPage', child),
  //     goalsSearchPage = _getHtml('goalsSearchPage', child, options),
  //     addGoalPager = _getHtml('pagerItem', child, addGoalOptions),
  //     searchPager = _getHtml('pagerItem', child, searchOptions),
  //     pager = _getHtml('pagerItem', child, options),
  //     myChildLinkInMenu = _getHtml('myChildLinkInMenu', child, options),
  //     cid = child.cid;

  //   // clear input fields
  //   $('#add-child-first-name').val('');
  //   $('#add-child-last-name').val('');
  //   $('#add-child-birth-date').val('');
  ...
  //   $('nav[data-role="panel"] li.search-holder')
  //     .after(myChildLinkInMenu);
  //   $('.ui-page nav[data-role="panel"] [data-role="listview"]')
  //     .listview('refresh');
  //   // Add child link to settings page.
  //   $('#my-childrens li:first').after(myChildren);
  //   $('.ui-page #my-childrens').listview('refresh');
  //   $('#settings.ui-page').trigger('create');
  //   // add child to front page
  //   $(article).prependTo('#list-children');
  //   // add child page after home page
  //   $('#home').after(childPage).after(goalsSearchPage).after(addGoalPage);
  //   $('#' + options.pagerName + cid).page();
  //   $('#' + searchOptions.pagerName + cid).page();
  //   $('#' + addGoalOptions.pagerName + cid).page();
  //   $('#home #main-menu').clone().prependTo('#' + options.pagerName + cid);
  //   $('#' + options.pagerName + cid).trigger('pagecreate');
  //   $('#home #main-menu').clone().prependTo('#' + searchOptions.pagerName + cid);
  //   $('#' + searchOptions.pagerName + cid).trigger('pagecreate');
  //   $('#home #main-menu').clone().prependTo('#' + addGoalOptions.pagerName + cid);
  //   $('#' + addGoalOptions.pagerName + cid).trigger('pagecreate');
  ...
  //   // update pager
  //   $(pager).prependTo('div[id*="' + options.pagerName + '"] ul.list-pagerer');
  //   $(searchPager).prependTo('div[id*="' + searchOptions.pagerName + '"] ' +
  //     'ul.list-pagerer');
  //   $(addGoalPager).prependTo('div[id*="' + addGoalOptions.pagerName + '"] ' +
  //     'ul.list-pagerer');
  //   // clone pager to new pages
  //   var $cPager = $('div[id*="' + options.pagerName + '"]:last ul.list-pagerer')
  //     .clone();
  //   var $sPager = $('div[id*="' + searchOptions.pagerName + '"]:last ' +
  //     'ul.list-pagerer').clone();
  //   var $adPager = $('div[id*="' + searchOptions.pagerName + '"]:last ' +
  //     'ul.list-pagerer').clone();
  //   $('#' + options.pagerName + cid).find('.child.item').append($cPager);
  //   $('#' + searchOptions.pagerName + cid).find('.child.item').append($sPager);
  //   $('#' + addGoalOptions.pagerName + cid).find('.child.item').append($adPager);

  //   $.mobile.loading('hide');

  //   _messagePopup('New Child has been created.', false);
  //   $.mobile.changePage('#children-' + cid, {
  //     transition: "slide",
  //     reverse: true
  //   });
  // }
  ...
  function _errorHandler(err) {
    $.mobile.loading('hide');
    _messagePopup('ERROR: ' + JSON.stringify(err), true);
  }
  ...
  function _getHtml(idx, dt, options) {
    ...
    switch (idx) {
      ...
      case 'childPage':
        ...
        output += '<li data-icon="plus"><a href="#edit-child" data-transition="slide">Edit child</a></li>';
        output += '</ul>';
  ...
  function _getStarted() {
    ...
    apApp.settings.dbPromiseTracker.transaction(function(tx) {
      tx.executeSql('INSERT INTO users (uid_origin, password, name, address, ' +
        'email, image_path, updated, created, status, update_photo) ' +
        'VALUES (0, ?, ?, ?, ?, ?, ?, ?, 1, 1)',
        [profile.password, profile.name, profile.address, profile.email, profile.image_path, ts, ts],
        function(tx, results) {
          // set user profile UID
  ...
  function _uploadContent() {
    ...
    apApp.settings.dbPromiseTracker.transaction(function(tx) {
      tx.executeSql('SELECT * FROM users WHERE uid = 1', [], _selectUploadUsers, _errorHandler);
    });
  }
  ...
  function _uploadUserChilds(user) {
    ...
    apApp.settings.dbPromiseTracker.transaction(function(tx) {
      tx.executeSql('SELECT c.*, ci.relationship, ur.uid_origin AS ruid_origin, r.title, u.uid_origin FROM childs AS c ' +
        ...
        'WHERE c.updated > ? AND c.uid = ?', [time, user.uid], _selectUploadChilds, _errorHandler);
    });
  }
  ...
  function _uploadUserGoals(user) {
    ...
    apApp.settings.dbPromiseTracker.transaction(function(tx) {
      tx.executeSql('SELECT g.*, u.uid_origin as author_id, a.age, a.delta AS age_delta, t.topic, t.delta AS topic_delta FROM goals AS g ' +
        ...
        'WHERE a.type="goal" AND t.type="goal" AND g.updated > ? AND g.uid = ?', [time, user.uid], _selectUploadGoals, _errorHandler);
    });
  }
  ...
  function _uploadGoalsofChildren() {
    ...
    apApp.settings.dbPromiseTracker.transaction(function(tx) {
      tx.executeSql('SELECT gi.gid, gi.completed, g.gid_origin, c.cid_origin, u.uid_origin FROM goal_index AS gi ' +
        'LEFT JOIN goals AS g ON g.gid = gi.uid ' +
        'LEFT JOIN childs AS c ON c.cid = gi.cid ' +
        'LEFT JOIN users AS u ON u.uid = gi.uid ' +
        'WHERE gi.updated > ?', [time], _selectUploadGoalsofChildren, _errorHandler);
    });
  }
